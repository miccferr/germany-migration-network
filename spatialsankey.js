  ...
  d3.spatialsankey = function() {
    ...
    spatialsankey.ranges = function() {
      ...
      nodes = nodes.map(function(node) {
        ...
        var inflows = links.filter(function(link) {
          return link.target == node.id;
        });
        var outflows = links.filter(function(link) {
          return link.source == node.id;
        });

        // Sum flows and set aggregate values
        node.properties.aggregate_inflows = inflows.reduce(function(memo, link) {
          return memo + link.flow;
        }, 0);
        node.properties.aggregate_outflows = outflows.reduce(function(memo, link) {
          return memo + link.flow;
        }, 0);
      ...
      link_flow_range.min = d3.min(links, function(link) {
        return link.flow;
      });
      link_flow_range.max = d3.max(links, function(link) {
        return link.flow;
      });
      ...
      node_flow_range.max = d3.max(nodes, function(node) {
        return node.properties.aggregate_outflows;
      });
    ...
    spatialsankey.links = function(_) {
      ...
      links = links.map(function(link) {
        ...
        var source_feature = nodes.filter(function(node) {
            return node.id == link.source;
          })[0],
          target_feature = nodes.filter(function(node) {
        ...
        var flow = flows.filter(function(flow) {
          return flow.id == link.id;
        })[0];
        if (flow) {
          link.flow = flow.flow;
        }
      ...
      if (link_count != links.length) {
        console.log(
          "Dropped " + (link_count - links.length) + " links that could not be matched to a node."
        );
      }
    ...
    spatialsankey.node = function(options) {
      ...
      node_color_range = ["#fff2bf", "#fdacb9"];
      ...
      node.r = function(d) {
        ...
        return (
          (node_radius_range.max - node_radius_range.min) * (diff / range) + node_radius_range.min
        );
      };
